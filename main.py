...
def lucky():
    if ( Whether_lucky!=False):
        try:
            ...
            data = res.read()
            data = json.loads(data)
            data = "爱情指数："+str(data["newslist"][1]["content"])+"   工作指数："+str(data["newslist"][2]["content"])+"     今日概述："+str(data["newslist"][8]["content"])
            return data
        except:
...
def tip():
    if (Whether_tip!=False):
        try:
            ...
            data = res.read()
            data = json.loads(data)
            print(data["newslist"][0]["uv_index"])
            print(data["newslist"][0]["tips"])
